...
<!--                    </template>-->
                    ...
                    <a-button type="primary" @click="handleSubmit" :disabled="loading" danger>
                        <template v-if="loading">
                            <a-spin /> Syncing...
                        </template>
                        <template v-else>
                            Sync Session - {{ importPeriods }}
                        </template>
                    </a-button>
                    <a-button type="primary" @click="handleStoreSessionData" :disabled="isloading" success>
                        <template v-if="isloading">
                            <a-spin /> Syncing...
                        </template>
                        <template v-else>
                            Sync Member-{{ importPeriods }}
                        </template>
                    </a-button>
...
<!--                                        <a-select-option v-for="period in importPeriods" :key="period" :value="period">-->
<!--                                            {{ period }}-->
<!--                                        </a-select-option>-->
<!--                                    </a-select>-->
<!--                                </a-form-item>-->
...
export default {
    ...
    setup() {
        ...
        const handleSubmit = async () => {
                ...
                try {
                    const response = await axiosAdmin.get('period-api-data-import-session/'+importPeriods.value);
                    console.log('API Response:', response.data.data);

                    if (response.status === 200) {
                        await setUrlData();
                        message.success(response.data.message || 'Data stored in session successfully.');
        ...
        const handleStoreSessionData = async () => {
                isloading.value = true;  // Set loading to true when the submit button is clicked
                try {
                    const response = await axiosAdmin.get('process-session-import-data');
                    console.log('API Response:', response.data.data);

                    if (response.status === 200) {
                        await setUrlData();
                        message.success(response.data.message || 'Data processed successfully.');
